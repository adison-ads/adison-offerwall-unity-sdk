...
class XCBuildConfiguration(PBXType):
    ...
    def add_runpath_search_paths(self, paths, recursive=True):
        return self.add_search_paths(paths, 'buildSettings', 'LD_RUNPATH_SEARCH_PATHS', recursive=recursive)

    def update_build_setting(self, key, value):
        base = 'buildSettings'
        if base not in self:
            self[base] = PBXDict()
        self[base][key] = value
...
class XcodeProject(PBXDict):
    ...
    def add_runpath_search_paths(self, paths, recursive=True):
        ...
        for b in build_configs:
            if b.add_runpath_search_paths(paths, recursive):
                self.modified = True
    ...
    def update_build_setting(self, key, value):
        ...
        for b in build_configs:
            if b.update_build_setting(key, value):
                self.modified = True
